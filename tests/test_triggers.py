...
from switchbot_exporter import triggers
...
def test_trigger_action_shell(mock_run, mock_advertisement_bot):
    trigger_config = {"type": "shell_command", "command": "echo 'Bot {address} pressed'"}
    triggers.trigger_action(trigger_config, mock_advertisement_bot)
    mock_run.assert_called_once_with("echo 'Bot DE:AD:BE:EF:22:22 pressed'", shell=True, check=False)
...
def test_trigger_action_webhook_failure(mock_log_error, mock_post, mock_advertisement_meter):
    mock_post.side_effect = requests.RequestException("Test connection error")
    trigger_config = {"type": "webhook", "url": "http://fail.example.com", "method": "POST"}
    triggers.trigger_action(trigger_config, mock_advertisement_meter)
